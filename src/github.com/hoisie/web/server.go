...
type Server struct {
	Config  *ServerConfig
	vroutes map[virtualHost]routeSet
	Logger  *log.Logger
...
func (s *Server) addRoute(r string, method string, handler interface{}, vHost string) {
	...
	_, ok := s.vroutes[virtualHost(vHost)]
	if !ok {
		s.vroutes[virtualHost(vHost)] = routeSet{}
	}

	switch handler.(type) {
	case http.Handler:
		s.vroutes[virtualHost(vHost)] = append(s.vroutes[virtualHost(vHost)], route{r: r, cr: cr, method: method, httpHandler: handler.(http.Handler)})
	case reflect.Value:
		fv := handler.(reflect.Value)
		s.vroutes[virtualHost(vHost)] = append(s.vroutes[virtualHost(vHost)], route{r: r, cr: cr, method: method, handler: fv})
	default:
...
func (s *Server) Get(route string, handler interface{}, vHost string) {
	s.addRoute(route, "GET", handler, vHost)
}

// Post adds a handler for the 'POST' http method for server s.
func (s *Server) Post(route string, handler interface{}, vHost string) {
	s.addRoute(route, "POST", handler, vHost)
}

// Put adds a handler for the 'PUT' http method for server s.
func (s *Server) Put(route string, handler interface{}, vHost string) {
	s.addRoute(route, "PUT", handler, vHost)
}

// Delete adds a handler for the 'DELETE' http method for server s.
func (s *Server) Delete(route string, handler interface{}, vHost string) {
	s.addRoute(route, "DELETE", handler, vHost)
}

// Match adds a handler for an arbitrary http method for server s.
func (s *Server) Match(method string, route string, handler interface{}, vHost string) {
	s.addRoute(route, method, handler, vHost)
}

//Adds a custom handler. Only for webserver mode. Will have no effect when running as FCGI or SCGI.
func (s *Server) Handler(route string, method string, httpHandler http.Handler, vHost string) {
	s.addRoute(route, method, httpHandler, vHost)
}

//Adds a handler for websockets. Only for webserver mode. Will have no effect when running as FCGI or SCGI.
func (s *Server) Websocket(route string, httpHandler websocket.Handler, vHost string) {
	s.addRoute(route, "GET", httpHandler, vHost)
}
...
func (s *Server) routeHandler(req *http.Request, w http.ResponseWriter) (unused *route) {
	requestPath := req.URL.Path
	vHost := virtualHost(sanitizeHostname(req.Host))
	ctx := Context{req, map[string]string{}, s, w, w.(http.Hijacker)}
	...
	_, ok := s.vroutes[virtualHost(vHost)]
	if !ok {
		if _, ok := s.vroutes[virtualHost(virtualHost(s.DefaultDomain))]; ok {
			vHost = virtualHost(s.DefaultDomain)
		} else {
			ctx.Abort(404, "Page not found for domain: "+string(vHost))
			return
		}
	}

	for i := 0; i < len(s.vroutes[virtualHost(vHost)]); i++ {
		route := s.vroutes[virtualHost(vHost)][i]
		cr := route.cr
