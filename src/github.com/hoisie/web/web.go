...
func Get(route string, handler interface{}, vHost string) {
	mainServer.Get(route, handler, vHost)
}

// Post adds a handler for the 'POST' http method in the main server.
func Post(route string, handler interface{}, vHost string) {
	mainServer.addRoute(route, "POST", handler, vHost)
}

// Put adds a handler for the 'PUT' http method in the main server.
func Put(route string, handler interface{}, vHost string) {
	mainServer.addRoute(route, "PUT", handler, vHost)
}

// Delete adds a handler for the 'DELETE' http method in the main server.
func Delete(route string, handler interface{}, vHost string) {
	mainServer.addRoute(route, "DELETE", handler, vHost)
}

// Match adds a handler for an arbitrary http method in the main server.
func Match(method string, route string, handler interface{}, vHost string) {
	mainServer.addRoute(route, method, handler, vHost)
}

//Adds a custom handler. Only for webserver mode. Will have no effect when running as FCGI or SCGI.
func Handler(route string, method string, httpHandler http.Handler, vHost string) {
	mainServer.Handler(route, method, httpHandler, vHost)
}

//Adds a handler for websockets. Only for webserver mode. Will have no effect when running as FCGI or SCGI.
func Websocket(route string, httpHandler websocket.Handler, vHost string) {
	mainServer.Websocket(route, httpHandler, vHost)
}
...
//Sets the host / virtualhost to use if blank/localhost
func SetDefaultDomain(in string) {
	mainServer.DefaultDomain = in
}
